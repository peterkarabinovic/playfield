...
def reproject_tif(path):
    ...
    with rasterio.Env():
        with rasterio.open(path, 'r') as src:
            ...
            clip_lb, clip_rt = crsWgs_to_src(
                [(clip_bounds.left, clip_bounds.bottom), (clip_bounds.right, clip_bounds.top)])
            ...
            dst_lb, dst_rt = crsWgs_to_tms(
                [(clip_bounds.left, clip_bounds.bottom), (clip_bounds.right, clip_bounds.top)])
            dst_width, dst_height = src.width, int(row_stop - row_start)
            dst_transform = [dst_lb[0], (dst_rt[0] - dst_lb[0]) / dst_width, 0, dst_rt[1], 0,
                             (dst_rt[1] - dst_lb[1]) / dst_height]
