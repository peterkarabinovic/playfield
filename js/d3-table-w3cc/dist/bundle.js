...
function d3Grid(div_id, options)
{
    var min = Math.min,
        max = Math.max,
        $div = select(div_id);
    ...
    var dispatch$$1 = dispatch('click','contextmenu','mouseenter', 'mouseleave');
    function eventHandlers($tr)
    {
        var handler = event_type => {
            return function(index){
                ...
                var tr = this;
                dispatch$$1.call(event_type, this, {
    ...
    function t(data)
    {
        ...
        if(npages > 1)
        {
            ...
            var scale = linear().domain([0, box.height-scrollthumb_h]).rangeRound([0, data.length-size]);
            styles($scrollthumb, {
                position: 'absolute',
                width: w+'px',
                top: '0px',
            ...
            $scrollthumb.call(d3drag().on('drag', function(){
                moveAndUpdate(event.dy);
            }));
            $scroll.on('click', function(){
                var sign = event.clientY > $scrollthumb.node().getBoundingClientRect().top ? 1 : -1;
    ...
    function tuneSizes(data)
    {
        ...
        $thead.selectAll('th').each(function(d,i){
            var width = chars[i] * w;
            table_w += chars[i] * w ;
            select(this).style('width', width +'px');
        });
        $table.style('width', table_w + 'px');
    }
    t = _.extend(t,dispatch$$1);
    return t;
}
function rowDecorator(d3grid, decorator_id)
{
    var $decorator = select(decorator_id);
